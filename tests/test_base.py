...
def assert_almost_equal(x, y, debug_msg='{}'):
    ...
    # assert np.allclose(x, y), debug_msg.format(f'wrong array value: els: {x[0, 0], y[0, 0]} \nmean{x.mean(0), y.mean(0)} \nstd: {x.std(0), y.std(0)}')
    assert np.allclose(x, y), debug_msg.format(f'wrong array value: elems: {x, y}')
...
def assert_almost_equal_or_zero(x, y, debug_msg):
    assert x.dtype == y.dtype, debug_msg.format('wrong array dtype')
    assert x.shape == y.shape, debug_msg.format('wrong array shape')
    assert ((y == 0) | np.isclose(x, y)).all(), debug_msg.format(f'wrong array value: {x[0, 0], y[0, 0]} \nmean{x.mean(0), y.mean(0)} \nstd: {x.std(0), y.std(0)}')
